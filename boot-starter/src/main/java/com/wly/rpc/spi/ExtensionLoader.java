...
/**
 * @author BarryCrude
 * @date 2025-01-26 0:27
 * @description: 一个 SPI 加载器，用于动态加载和管理扩展实现类
 */
public class ExtensionLoader {
    ...
    public <V> V get(String name) {
        if (!singletonsObject.containsKey(name)) {
            // todo 视情况是否try catch
            singletonsObject.put(name, extensionClassCache.get(name));
        }
    ...
    public void loadExtension(Class clazz) throws IOException, ClassNotFoundException {
        if (clazz == null) {
            throw new IllegalArgumentException("Class类没找到");
        }
